<div class="space-y-5 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between pt-1">
        ...
        <div class="flex items-center gap-2">
            @if($currentBusinessDay && $currentBusinessDay->isOpen())
                <span
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-emerald-500/15 text-emerald-400 border border-emerald-500/30">
                    <span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                    <span>Cart Open</span>
                </span>
            @else
                <span
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-rose-500/15 text-rose-400 border border-rose-500/30">
    ...
    <div
        class="bg-gradient-to-br from-zinc-900 via-zinc-900 to-zinc-950 border {{ $isTodayProfit ? 'border-emerald-500/30' : 'border-rose-500/30' }} rounded-3xl p-5 sm:p-6 shadow-2xl relative overflow-hidden">
        ...
        <div class="grid grid-cols-2 gap-4 py-4">
            ...
            <div class="bg-zinc-950/70 border border-zinc-800/70 rounded-2xl p-3.5 flex flex-col justify-between">
                ...
                <div
                    class="text-2xl sm:text-3xl font-black {{ $isTodayProfit ? 'text-emerald-400' : 'text-rose-400' }} tracking-tight mt-1">
        ...
        @if($isTodayProfit)
            <div
                class="mt-2 py-2.5 px-4 rounded-xl bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-between text-xs sm:text-sm font-bold text-emerald-300">
                <div class="flex items-center gap-2">
        ...
        @else
            <div
                class="mt-2 py-2.5 px-4 rounded-xl bg-rose-500/15 border border-rose-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs sm:text-sm font-bold text-rose-300">
                <div class="flex items-center gap-2">
    ...
    <div class="space-y-2">
        ...
        <div class="grid grid-cols-2 gap-3 sm:gap-4">
            ...
            <a href="{{ route('admin.sales') }}"
                ...
                <span class="text-[11px] text-zinc-500 mt-0.5">Today:
                    {{ $currency }}{{ number_format($todaySales, 0) }}</span>
            </a>
            ...
            <a href="{{ route('admin.expenses') }}"
                ...
                <span class="text-[11px] text-zinc-500 mt-0.5">Today:
                    {{ $currency }}{{ number_format($todayExpenses, 0) }}</span>
            </a>
    ...
    <div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 sm:p-6 shadow-xl">
        ...
        <div class="grid grid-cols-3 gap-3 pt-4 text-center sm:text-left">
            ...
            <div class="bg-zinc-950/60 border border-zinc-800/60 rounded-2xl p-3">
                ...
                <span
                    class="text-base sm:text-xl font-black {{ $monthProfit >= 0 ? 'text-amber-400' : 'text-rose-400' }} mt-0.5 block">
    ...
    <div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 sm:p-6 shadow-xl space-y-3">
        ...
        <div class="space-y-2 pt-1">
            @forelse($bestSellingItems as $item)
                <div
                    class="bg-zinc-950/80 border border-zinc-800/80 rounded-2xl p-3.5 flex items-center justify-between gap-3 text-xs sm:text-sm">
                    ...
                    <div class="flex items-center gap-3 text-right shrink-0">
                        ...
                        <span
                            class="font-black text-emerald-400">{{ $currency }}{{ number_format($item->total_revenue, 0) }}</span>
                    </div>
                </div>
            @empty
    ...
    <div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 shadow-xl space-y-4">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-2.5">
                <div
                    class="w-10 h-10 rounded-2xl {{ $currentBusinessDay && $currentBusinessDay->isOpen() ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-rose-500/20 text-rose-400 border border-rose-500/30' }} flex items-center justify-center text-lg shrink-0">
                ...
                <div>
                    <h4 class="font-black text-sm sm:text-base text-white flex items-center gap-2">
                        ...
                        <span
                            class="{{ $currentBusinessDay && $currentBusinessDay->isOpen() ? 'text-emerald-400' : 'text-rose-400' }}">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
            ...
            <div class="bg-zinc-950/80 border border-zinc-800/80 rounded-2xl p-3.5 flex items-center justify-between">
                <div>
                    ...
                    <span
                        class="text-base sm:text-lg font-black {{ $currentBusinessDay?->closed_at ? 'text-rose-400' : 'text-amber-400' }} mt-0.5 block">
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div class="bg-zinc-900 border border-zinc-800 rounded-3xl w-full max-w-md p-5 sm:p-6 shadow-2xl space-y-4">
                ...
                <form wire:submit="closeBusinessDay" class="space-y-3.5">
                    <div>
                        <label class="block text-xs font-semibold text-zinc-300 mb-1">Final Cash in Register
                            ({{ $currency }})</label>
                    ...
                    <div>
                        ...
                        <textarea wire:model="dayNotes" rows="2" placeholder="Shift notes or remarks..."
                            class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-2 text-xs text-white focus:ring-2 focus:ring-amber-500 focus:outline-none"></textarea>
                    </div>

                    <div class="flex items-center justify-end gap-2 pt-2 border-t border-zinc-800">
                        <button type="button" wire:click="$set('showDayModal', false)"
                            class="px-4 py-2 rounded-xl text-xs font-semibold text-zinc-400 hover:text-white bg-zinc-800">
                            Cancel
                        </button>
                        <button type="submit"
                            class="px-4 py-2 rounded-xl text-xs font-bold text-white bg-rose-600 hover:bg-rose-500">
                            Confirm & Close Day
                        </button>
                    </div>
                </form>
