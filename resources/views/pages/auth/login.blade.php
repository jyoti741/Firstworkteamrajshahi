<x-layouts::auth :title="__('CartFlow Login')">
    <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col items-center text-center gap-2">
            <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-amber-500 via-orange-500 to-amber-600 flex items-center justify-center text-3xl shadow-xl shadow-amber-500/20 border border-amber-400/30 mb-1">
                🍔
            </div>
        ...
        <form method="POST" action="{{ route('login.store') }}" class="flex flex-col gap-4">
            ...
            <div>
                ...
                <div class="relative">
                    <input
                        ...
                        name="email"
                        type="email"
                        value="{{ old('email', 'admin@cartflow.test') }}"
                        required
                        autofocus
                        autocomplete="email"
            ...
            <div>
                <div class="flex items-center justify-between mb-1.5">
                    ...
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-xs text-amber-400 hover:text-amber-300">Forgot password?</a>
                    @endif
                </div>
                <div class="relative">
                    <input
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" id="remember" name="remember" class="rounded bg-zinc-900 border-zinc-700 text-amber-500 focus:ring-amber-500 h-4 w-4" checked>
                <label for="remember" class="text-xs text-zinc-400">Remember on this device</label>
            </div>

            <!-- Submit Button -->
            <button type="submit" class="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-zinc-950 font-bold text-sm shadow-lg shadow-orange-500/25 transition-all transform active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2">
                <span>🔐</span>
